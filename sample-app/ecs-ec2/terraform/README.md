# Terraform Deployment for Nest.js App on ECS EC2 Spot Instances

## Overview

This Terraform configuration provisions the necessary AWS infrastructure to deploy a Nest.js application on ECS EC2 Spot Instances with an Application Load Balancer (ALB) fronting the service. The key features of this configuration include:
...
- **ECS Resources:**
  ...
  - An ECS Task Definition with container definitions for your Nest.js app and supporting containers
  - An ECS Service that registers with the ALB target group

- **EC2 Spot Instances:**
  - An Auto Scaling Group (ASG) that provisions EC2 Spot Instances using a Launch Template
  - EC2 instances are registered to the ECS cluster and run your tasks cost-effectively
...
flowchart TD
    ...
    EC2ASG["EC2 Auto Scaling Group (Spot Instances)"]
    EC2Instance["EC2 Spot Instance"]
    ECSTask["Task Definition (Nest.js App & Agents)"]
